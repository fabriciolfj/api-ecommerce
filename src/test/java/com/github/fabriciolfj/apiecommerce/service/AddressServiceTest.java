...
import com.github.fabriciolfj.apiecommerce.service.impl.AddressServiceImpl;
...
public class AddressServiceTest {
  ...
  public void convertModelToEntity() {
    // given
    AddressConverter srvc = new AddressConverter();
    // when
  ...
  public void deleteAddressesByNonExistId() throws Exception {
    Mockito.doThrow(new ResourceNotFoundException(String.format("No Address found with id %s.", nonExistId)))
            .when(repository).deleteById(UUID.fromString(nonExistId));
    // when
    try {
      service.deleteAddressesById(nonExistId);
    } catch (Exception ex) {
